...
function unbracketTag(str) {
    return str.replace(/<(.*)>/, '$1'); // str.slice(1, -1);
}
...
function getRectangleString(width, height) {
    let result = '┌'.padEnd(width - 1, '─').concat('┐\n');

    if (height - 2 > 0) {
        result += '│'.padEnd(width - 1, ' ').concat('│\n').repeat(height - 2);
    }
            ...
            .map(
                (character) =>
                    alphabet.indexOf(character) !== -1
                        ? rotatedAlphabet[alphabet.indexOf(character)]
                        : character
            ).join('');
*/
    ...
    for (const character of str) {
       result += alphabet.indexOf(character) !== -1
           ? rotatedAlphabet[alphabet.indexOf(character)]
           : character
    }
